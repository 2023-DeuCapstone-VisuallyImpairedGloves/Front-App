...
import com.deucapstone2023.ui.screen.search.SearchViewModel
import com.deucapstone2023.ui.screen.search.state.Location
import com.deucapstone2023.ui.screen.setting.SettingViewModel
import com.deucapstone2023.ui.screen.setting.state.ButtonStatus
import com.deucapstone2023.ui.screen.setting.state.toBoolean
...
class MainActivity : ComponentActivity() {
        ...
        lifecycleScope.launch {
            ...
            launch {
                while (true) {
                    while (flag) {
                        if (!flag2)
                            searchViewModel::setUserLocation.invoke(
                                searchUiState.latitude + 0.00010000000000,
                                searchUiState.longitude - 0.00001000000000
                            )
                        else
                            searchViewModel::setUserLocation.invoke(
                                searchUiState.latitude + 0.00000050000000,
                                searchUiState.longitude - 0.0001000000000
                            )
    ...
    private fun doOnPermissionApproved() {
        initState()
        setContent {
            com.deucapstone2023.design.theme.DeuCapStone2023Theme {
                Content()
            }
        }
        subscribeUI()
    ...
    private fun setSpeechRecognizerListener(
        listener: com.deucapstone2023.ui.CommonRecognitionListener
    ) {
