package com.deucapstone2023.data.data.datasource.remote.mapper

import com.deucapstone2023.data.data.datasource.remote.dto.response.poi.POIResponse
import com.deucapstone2023.data.data.datasource.remote.dto.response.poi.Poi
...
import com.deucapstone2023.domain.domain.utils.getDistanceFromSourceToDest
    ...
    this.features?.forEachIndexed { index, feature ->
        when (feature.geometry?.type) {
            ...
            "LineString" -> {
                ...
                routeFacilityType = if (feature.properties?.facilityType?.isNotBlank() == true) feature.properties.facilityType.toInt() else 0
                totalDistance = getDistanceFromSourceToDest(startLatitude, startLongitude, destinationLatitude, destinationLongitude)
                totalTime = feature.properties?.time ?: 0
                lineInfo.addAll(location?.map {
                    val eachLocation = (it as List<*>)
                    LineModel(
                        eachLocation[1] as Double,
                        eachLocation[0] as Double
                    )
                } ?: emptyList())
            }
        }
