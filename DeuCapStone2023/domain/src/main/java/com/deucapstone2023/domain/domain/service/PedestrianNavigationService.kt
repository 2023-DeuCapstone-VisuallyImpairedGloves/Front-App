...
import com.deucapstone2023.domain.domain.utils.getCurrentTime
import javax.inject.Inject
import kotlin.math.atan2
import kotlin.math.cos
import kotlin.math.sin

class PedestrianNavigationService @Inject constructor(
    ...
    override fun getDistanceFromSourceToDest(source: LineModel, dest: LineModel): Int =
        com.deucapstone2023.domain.domain.utils.getDistanceFromSourceToDest(
    ...
    override suspend fun navigateRouteOnMap(
        ...
        if (recentDistance - getDistanceFromSourceToDest(
                source = source,
            ...
            if (recentDistance > getDistanceFromSourceToDest(
                ...
                recentDistance = getDistanceFromSourceToDest(
                    source = source,
                    dest = LineModel(route.destinationLatitude, route.destinationLongitude)
                )
                ...
                if (recentDistance >= halfPoint - 10 && recentDistance <= halfPoint + 10) {
                    ...
                    if (getDistanceFromSourceToDest(
                            source = source,
                            dest = route.lineInfo[recentLineInfoIndex + 1]
                        ) < 10
                    )
